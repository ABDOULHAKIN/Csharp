...
namespace TpLinq {
    public class Donnees {
        ...
        public void Requetes() {
            ...
            foreach(Livre livre in ListeLivres.Where(l => l.Auteur.Nom == "RICHARD" && l.Auteur.Prenom == "Thierry")) {
                Console.WriteLine(" - "+livre.Titre);
            }
            // 2 - Afficher la liste des noms des auteurs dont le prénom se termine par "y"
            Console.WriteLine("2 - Afficher la liste des noms des auteurs dont le prénom se termine par \"y\"");
            foreach(String nom in ListeAuteurs.Where(a => a.Prenom.EndsWith("y")).Select(a => a.Nom)) {
                Console.WriteLine(" - "+nom);
            }
            // 3 - Afficher les titres de tous les livres triés par ordre alphabétique
            Console.WriteLine("3 - Afficher les titres de tous les livres triés par ordre alphabétique");
            foreach(Livre livre in ListeLivres.OrderBy(l => l.Titre)) {
                Console.WriteLine(" - "+livre.Titre);
            }
            ...
            foreach(Auteur auteur in ListeLivres.Where(l => l.NbPages > 400).Select(l => l.Auteur).Distinct()) {
            ...
            double nbPagesMoy = ListeLivres.Average(l => l.NbPages);
            Console.WriteLine($"En moyenne un livre possède {nbPagesMoy} pages");
            foreach (Livre l in ListeLivres.Where(li => li.NbPages > nbPagesMoy)) {
                Console.WriteLine(" - " + l.Titre);
            }
            // 6 - Le livre ayant le titre le plus long
            Console.WriteLine("6 - Le livre ayant le titre le plus long");
            Livre? livreTitreLong = ListeLivres.OrderByDescending(l => l.Titre.Length).FirstOrDefault();
            if (livreTitreLong != null) {
                Console.WriteLine(" -> " + livreTitreLong.Titre);
            } else {
            ...
            String? titrePlusPages = ListeLivres.OrderByDescending(l => l.NbPages).Select(l => l.Titre).FirstOrDefault();
            if(titrePlusPages != null)
                Console.WriteLine(" -> "+titrePlusPages);
            else
                Console.WriteLine("  --  aucun livre --");
            // 8 - Afficher les auteurs et la liste de leurs livres
            Console.WriteLine("8 - Afficher les auteurs et la liste de leurs livres");
            foreach (var auteurAvecLivres in ListeLivres.GroupBy(l => l.Auteur,
                (auteur, ensLivre) => new {
                    PrenomNomAuteur = auteur.Prenom + " " + auteur.Nom,
                    Livres = ensLivre
                })) {
                Console.WriteLine(" - " + auteurAvecLivres.PrenomNomAuteur);
                foreach (Livre livre in auteurAvecLivres.Livres) {
                    Console.WriteLine("    - " + livre.Titre);
                }
            }
            ...
            foreach(var groupe in ListeLivres.GroupBy(l => l.Auteur,
                (auteur, livres) => new {
                    Auteur = auteur,
                    NbPagesEnMoyenne = livres.Average(l => l.NbPages)
                }))
                Console.WriteLine($" - {groupe.Auteur.Prenom} {groupe.Auteur.Nom} -> {groupe.NbPagesEnMoyenne} pages");
            // 10 - Afficher l’auteur ayant écrit le plus de livres
            Console.WriteLine("10 - Afficher l’auteur ayant écrit le plus de livres");
            var autNbLivres = ListeLivres
                .GroupBy(l => l.Auteur, (auteur, livres) => new { Auteur = auteur, NbLivres = livres.Count() })
                .OrderByDescending(g => g.NbLivres);
            int maxLivres = autNbLivres.Max(g => g.NbLivres);
            foreach(Auteur a in autNbLivres.Where(g => g.NbLivres == maxLivres).Select(g => g.Auteur)) {
            ...
            decimal moyenneFacyure = ListeAuteurs.SelectMany(a => a.Factures).Average(f => f.Montant);
            Console.WriteLine(" -> "+moyenneFacyure);
